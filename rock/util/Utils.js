define(function(require) {
	...
	*/ 
	...
	/*
	*判断精灵队列之间的碰撞  若碰撞  则默认删除B的元素 若type为真则全部删除
	*@param objectA 精灵队列A
	*@param objectB 精灵队列B
	*/ 
	Utils.TestArrayDisplay = function(objectA,objectB,type,npc3){
		for(var i=0;i<objectA.length;i++){
			for(var j=0;j<objectB.length;j++){
				if(Utils.hitTestObject(objectA[i],objectB[j])){
					objectB.removeAt(j);
					npc3.scoreEvent();
					if(type){
						objectA.removeAt(i);
	...
	/*
	*判断精灵队列与精灵的碰撞  若碰撞  则默认删除B 若type为真则全部删除
	*@param objectA 精灵队列A
	*@param objectB 精灵B
	*/ 
	Utils.TestArraytoObject = function(array,object,type){
		for(var i=0;i<array.length;i++){
			if(Utils.hitTestObject(array[i],object)){
				object.onHitEvent();
				if(type){
					array.removeAt(i);
				}
	...
	*@param objectB 精灵B
	*/
	Utils.clearArrayObject = function(array){
		for(var i=0;i<array.length;i++){
			if(array[i].name == 'airplane' || array[i].name == 'bee'){
				if(array[i].y >= 500){
					array.removeAt(i);
				}
			}
			if(array[i].name == 'bullet'){
				if(array[i].y <=0){
					array.removeAt(i);
				}
			}
	...
	*@param objectB 精灵B
	*/ 
	Utils.PaintText = function(ctx,text,x,y){
		ctx.context.font = "15px Courier New";
		ctx.context.fillStyle = 'black';
		ctx.context.fillText(text,x,y);
	};
